...
def HistogramMaker(CDResults):
    ...
    ax1.bar(range(len(counter)), counter.values(), align="center")
    ax1.set_xticks(range(len(counter)))
    ax1.set_title("CDelement Frequency")
    ax1.set_ylabel("No. of materials")
    ax1.set_xlabel("Charge disproportionation elements")
    
    plt.xticks(range(len(counter)), [key[0:5] for key in list(counter.keys())])
    plt.tight_layout()
    plt.savefig("NonRadSearch.png", dpi=300, bbox_inches="tight")
...
CDResults = ReadJSONFile("NonRadSearchCDCandidates.json")
